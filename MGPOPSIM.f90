...
!	real(4)					:: h2			! heritability
 ...
 !== lixiujin added===
   integer                  :: ntrait
   integer                  :: tqtl1,tqtl2,tqtl3  ! tqtl1(++ or --),tqtl2(+/- 0),tqtl3(0 +/-)
   integer                  :: ind_var     ! 0 to be continue trait ,1 to be threshold trait
   real*4,allocatable       :: va2(:)
   real*4                   :: qtleff_r1,qtleff_r2,ra,pre_ra ! r3
   real*4                   :: re
...
!=============== Program start ======================
call show(io_mode , 'GPOPsim ___________Last modified 2011/03/07')
		...
		call changepara(2) 
			...
			if (mode==1) then
			   ...
			   if(ind_var==1)  then        !  simulate threshold  
                 ...
                 if(ntrait==2)   T=norminv(Tr,0.0,sqrt(va2(1)/h2(1)))
...
SUBROUTINE WELCOMEINF
    ...
    call show(io_mode, "          |Function module(two correlated traits)|")             
    call show(io_mode, "          |                 by Xiujin Li 2013/09 |")
...
implicit none
    ...
    if(io_mode==1) then
      ...
      parafile=trim(parafile)
		...
		open(11, file=parafile)
		    read (11,*) !line1
			read (11,*) ntrait,ind_var,Tr
			read (11,*) !line 2
			read (11,*) nmark, nchr, markrule
			read (11,*) !line 3
			read (11,*) lenchr, mratem, mrateq 
			read (11,*) !line 4
			read (11,*) qtlrule, nqtl, qtlp1, qtlp2,qtleff_r1,qtleff_r2,qtleff_r2
			read (11,*) !line 5
			read (11,*) h2(1),h2(2),pre_ra,re
			read (11,*) !line 6
	...
	allocate(rrate(2,tq))
	allocate(bpv(psize,2,2))  ! lixiujin eidted
	mrec=0
	qrec=0 
	mrec(:,:)=2
	qrec(:,:)=2	
...
implicit none
	...
	!==lixiujin added=====                for gamma distribution             
	 ...
	 real*8                 :: x1,x2,x3,u1
	 real*4                 :: rq=0.0,corr_rq=0.0 ! the correlation of qtl effect
...
!			if (u <0.5D0) vq(i)=-1*vq(i)
   ...
   if (ntrait==2) then
     if(qtleff_r1/=0.0) then 
	   rq=pre_ra*(qtleff_r1+qtleff_r2)/qtleff_r1
	   if(rq>1.0) then
	      write(*,*) 'rq larger than 1.0 to adjust QTL proportion(Group1,Group2,Group3)!'
		  stop
	   end if
     end if
    ...
    if(ntrait==2) then
	 ...
	 do i=1,tqtl
	   if(i<=qtleff_r1*tqtl) then
	      ...
	      x1=gamma(0.4*rq,qtlp2)
	      x2=gamma(0.4*(1.0-rq),qtlp2)
	      x3=gamma(0.4*(1.0-rq),qtlp2)
        vq(i,1)=x1+x2
		vq(i,2)=x1+x3
		u=randz()
        if(u<= 0.25E0) then
		  vq(i,1)=-1.0E0*vq(i,1)
  	      vq(i,2)=-1.0E0*vq(i,2)
		else if(u>0.25E0.and.u<= 0.50E0) then
		  vq(i,1)=-1.0E0*vq(i,1)
		  vq(i,2)=vq(i,2)
        else if(u>0.50E0.and.u<= 0.75E0) then
		  vq(i,1)=vq(i,1)
		  vq(i,2)=-1.0E0*vq(i,2)
	    endif
...
! control rq
  if((ntrait==2).and.(rq/=0.0)) then
    if(allocated(x)) deallocate(x)
    if(allocated(y)) deallocate(y)
	ncommon=qtleff_r1*tqtl
    allocate(x(ncommon),y(ncommon))
	x=vq(1:ncommon,1);y=vq(1:ncommon,2)
	call correl(x,y,ncommon,corr_rq)
   if(((rq-0.05)<=corr_rq).and.(corr_rq<=(rq+0.05))) then
      call show(io_mode,'producing the right correlation of qtl effect!',1)
	  write(*,*) rq,corr_rq, round
	  deallocate(x,y)
   else
      round=round+1
	  if(round>=10000) then
	    call show(io_mode,'put new seed or changing ratio of qtl effect!',1)
		stop
	  endif
      goto 1003
   end if
...
tvg(:)=0.0E0
 ...
 if(ntrait==1) then
    do i=1, tqtl
	   ...
	   tvg(1)=tvg(1)+2.0E0*freq*(1.0E0-freq)*vq(i,1)*vq(i,1)
	end do
	vq(:,1)=sqrt(1.0E0/tvg(1))*vq(:,1)	!rescal qtl variance to 1
 end if
 ...
 if(ntrait==2) then
	do i=1, tqtl
		 freq=real(count(qtype(qtlid(i),:)==1))/real(gsize) !get qtl gene freq.
		 tvg(1)=tvg(1)+2.0E0*freq*(1.0E0-freq)*vq(i,1)*vq(i,1)
         tvg(2)=tvg(2)+2.0E0*freq*(1.0E0-freq)*vq(i,2)*vq(i,2)  
    end do
    vq(:,1)=sqrt(1.0E0/tvg(1))*vq(:,1)
	vq(:,2)=sqrt(2.0E0/tvg(2))*vq(:,2)
   do i=1, tqtl
	   freq=real(count(qtype(qtlid(i),:)==1))/real(gsize) !get qtl gene freq.
       tvg(3)=tvg(3)+2.0E0*freq*(1.0E0-freq)*vq(i,1)*vq(i,2)
   end do
   
     allocate(va2(2))
     va2(1)=1.0;va2(2)=2.0
     ra=tvg(3)/sqrt(va2(1)*va2(2))
    
	 u=va2(1)*(1.0E0/h2(1)-1.0E0)    ! the residual variance for T1
	 u1=va2(2)*(1.0E0/h2(2)-1.0E0)   ! the residual variance for T2
 
	! write(*,*) va2(1),va2(2),tvg(3),ra
     
	open(unit=21,file="GP_2.out",status="replace")
	   write(21,"(' ','	','T1','	','T2')")
	   write(21,"('Va:','	',f,'	',f)") va2(1),va2(2)
       write(21,"('Ve:','	',f,'	',f)") u,u1
	   write(21,"('h2:','	',f,'	',f)") h2(1),h2(2)
	   write(21,"('Genetic Covariance:','  ',f,'	','Residual Covariance:','  ',f)") tvg(3),re*sqrt(u*u1)
       write(21,"('ra:','  ',f,'	','re:','  ',f)") ra,re
    close(21)   	 	    
 end if
...
implicit none
 ...
 !== lixiujin added===
	...
	if(allocated(stdve)) deallocate(stdve)
	allocate(stdve(2))
	...
	allocate(bpv(psize,2,2))

	if(ntrait==1)  then
      stdve(1)=sqrt(1.0E0/h2(1)-1.0E0)        ! residual variance =va(1.0/h2-1.0)
	  bpv=0.0E0
	  do i=1, psize
		tbv=0.0E0
...
!			end select
		...
		u=stdnorm()
		bpv(i,2,1)=u*stdve(1)+tbv(1)	! phenotype value
!		write(*,"(3F8.5)") tbv,u*stdve,bpv(i,2)
	 end do
   end if
  
   if(ntrait==2) then                              !lixiujin added
     stdve=va2*(1.0E0/h2-1.0E0) 
     allocate(V(2,2),e(2))
	 V=0.0;e=0.0
     V(1,1)=stdve(1)
	 V(2,2)=stdve(2)
	 V(1,2)=re*sqrt(V(1,1)*V(2,2))
	 V(2,1)=V(1,2)
	...
	do i=1, psize
		...
		do j=1,tqtl		! tbv  phenotype
			...
			genotype =genotype - 3
		  	 ...
		  	 tbv(1)=tbv(1)+ vq(j,1)*genotype
	         tbv(2)=tbv(2)+ vq(j,2)*genotype
		end do
		bpv(i,1,1)=tbv(1)			! true breeding value
		bpv(i,1,2)=tbv(2)
         
        call multinorm(V,2,e)
        bpv(i,2,1)=e(1)+tbv(1)
	    bpv(i,2,2)=e(2)+tbv(2)
...
implicit none
	...
	integer(4)			:: i,j,g
		...
		if(mode==1) then
		  ...
		  if(ntrait==2) then
		    write (22, "(3A7,2A5, 4A10)") "ID", "Sire", "Dam", "Sex","Bred", "TBV1", "Phe1","TBV2", "Phe2"
		  end if
		...
		do i=1, psize	!breeding value, pedigree output
			...
			if (mode==1) then
			 ...
			 if(ntrait==2) then
				   write (22,"(3(2x I6.6),2(I3),2(2x F18.12),2(2x F18.12))") ido,ids,idd, &
		     	 & 2-mod(i, 2), bred, bpv(i,1,1), bpv(i,2,1), bpv(i,1,2), bpv(i,2,2)
			 end if
...
implicit none
	...
	if(ntrait==2) then
	    write(23,*)'++ or --:',tqtl1,'  +/- 0:',tqtl2,'  0 +/-:',tqtl3
    end if
...
!		write (23,"(A10,I5,2x F9.6)")  name, i, vq(i)!
      ...
      if(ntrait==2) then 
		write (23,'(I6, F18.12, F18.12)') qtlid(i), vq(i,1),vq(i,2)
	  end if
   end do
